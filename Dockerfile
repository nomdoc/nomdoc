...
ARG DEBIAN_DATED_VERSION=bullseye-20231009-slim
...
ARG ELIXIR_IMAGE="hexpm/elixir:${ELIXIR_VERSION}-erlang-${OTP_VERSION}-debian-${DEBIAN_DATED_VERSION}"
ARG RUNNER_IMAGE="debian:${DEBIAN_DATED_VERSION}"
...
COPY --from=elixir-builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/nomdoc ./
